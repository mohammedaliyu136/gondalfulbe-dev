...
@endpush
@section('page-title')
    {{__('Payslip-Detail: Transport Crew')}}
@endsection
@section('breadcrumb')
            ...
            <div class="row mb-2">
                @if(\Auth::user()->can('approve payment rider'))
                    ...
                    <div class="col-auto float-end mt-4">
                        ...
                        <!--</a> -->
                        <a href="{{ route('rider.payslips.approvepayment', \Crypt::encrypt($payslip->id)) }}" 
                ...
                @if(\Auth::user()->can('initialise payment rider'))
                     @if($payslip->status == 1)
                        <div class="col-auto float-end mt-4">
                            <a href="{{route('rider.payslips.initialisepayment', \Crypt::encrypt($payslip->id))}}" class="btn btn-info"
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="p-3 bg-light rounded">
                        ...
                        <p>
                            @if(\Auth::user()->can('initialise payment rider'))
                            <a href="{{route('rider.payslips.revalidate', \Crypt::encrypt($payslip->batch_reference))}}" class="btn btn-sm btn-info"
                ...
                @foreach ($payslipItems as $payslipItem)
                    <tr>
                        <td class="vender_name">
                            <a href="{{ route('rider.show', \Crypt::encrypt($payslipItem->rider_id)) }}" class="btn btn-outline-primary">
                                {{ $payslipItem->rider->name }} <!-- Access vendor's name -->
                            </a>
                        </td>
                        <td>
                        ...
                        <td class="Action">
                            ...
                            <div class="action-btn me-2">
                                <a href="#"
                                           data-url="{{ route('rider.payslips.reverse.page', \Crypt::encrypt($payslipItem->id)) }}" 
...
<div class="modal fade" id="otpModal" tabindex="-1" aria-labelledby="otpModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form id="otpForm" method="POST" action="{{route('rider.payslips.authorisepayment')}}">
...
$(document).ready(function () {
    $('#otpForm').on('submit', function (e) {
        ...
        $.ajax({
            url: '{{ route('rider.payslips.authorisepayment') }}',
            type: 'POST',
            headers: {
    ...
    $('#resend-token').on('click' , function() {
        var reference = $(this).data('ref');
         $.ajax({
            url: '{{ route('rider.payslips.resendtoken') }}',
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
                ...
                if(errorMessage == 'Transaction batch not awaiting authorization'){
                    ...
                    if(ans == 'yes'){
                        ...
                        alert('Transaction is reinitialising. check your email for OTP');
                         window.location.href = "{{route('rider.payslips.initialisepayment', \Crypt::encrypt($payslip->id))}}";
                    }else if (ans == 'no'){
