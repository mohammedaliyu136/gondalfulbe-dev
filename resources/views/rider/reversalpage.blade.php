<div class="card shadow-lg border-0">
    ...
    <div class="card-body">
        <h4 class="text-dark text-center mb-4">{{ $payslip->rider->name }}</h4>
        ...
        <div class="table-responsive">
            <table class="table table-striped">
                <tbody>
                    <tr>
                        <th>Amount:</th>
                        <td><span class="text-success">₦{{ number_format($payslip->amount, 2, '.', ',') }}</span></td>
                    </tr>
                    <tr>
                        <th>Transaction Reference:</th>
                        <td class="text-muted">{{ $payslip->reference }}</td>
                    </tr>
                    <tr>
                        <th>Error Description:</th>
                        <td class="text-muted">{{ $payslip->txn_description }}</td>
                    </tr>
                </tbody>
            </table>
        </div>

        <form id="reversal-form" action="{{ route('rider.payslips.process.reversal') }}" method="POST">
            @csrf
            ...
            <div class="text-end">
                <button type="submit" class="btn btn-primary" id="submit-button" >
                    <span id="button-text">Process Reversal</span>
