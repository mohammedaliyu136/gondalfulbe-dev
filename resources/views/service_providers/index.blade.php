...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{__('Service Providers')}}</li>
@endsection
@section('action-btn')
    <div class="float-end d-flex">
        ...
        @can('create service provider')
            <a href="#" data-size="lg" data-url="{{ route('service-provider.create') }}" data-ajax-popup="true" data-title="{{__('Create New Service Provider')}}" data-bs-toggle="tooltip" title="{{ __('Create') }}" class="btn btn-sm btn-primary">
                            ...
                            <tbody>
                                @foreach ($venders as $k => $Vender)
                                    <tr class="cust_tr" id="vend_detail">
                                        <td class="Id">
                                            @can('show vender')
                                            <a href="{{ route('service-provider.show', \Crypt::encrypt($Vender['id'])) }}" class="btn btn-outline-primary">#SP0{{$Vender['vender_id']}}
                                                </a>
                                            @else
                                                <a href="#" class="btn btn-outline-primary">#SP0{{$Vender['vender_id'] }}</a>
                                            @endcan
                                        </td>
                                        <td>{{ $Vender['name'] }}</td>
                                        <td>{{ $Vender['contact'] }}</td>
                                        <td>{{ $Vender['billing_city'] }}</td>
                                        <td class="Action">
                                            <span>
                                                    @if ($Vender['is_active'] == 0)
                                                        <i class="fa fa-lock" title="Inactive"></i>
                                                    @else
                                                        @can('show service provider')
                                                            <div class="action-btn me-2">
                                                                <a href="{{ route('service-provider.show', \Crypt::encrypt($Vender['id'])) }}"
                                                        ...
                                                        @can('edit service provider')
                                                            <div class="action-btn me-2">
                                                                ...
                                                                data-title="{{__('Edit Vendor')}}"
                                                                    data-url="{{ route('service-provider.edit', $Vender['id']) }}"
                                                        ...
                                                        @can('delete service provider')
                                                            <div class="action-btn ">
                                                                {!! Form::open(['method' => 'DELETE', 'route' => ['service-provider.destroy', $Vender['id']], 'id' => 'delete-form-' . $Vender['id']]) !!}
