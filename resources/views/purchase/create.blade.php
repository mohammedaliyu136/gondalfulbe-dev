...
@push('script-page')
    ...
    var colcentre = $(this).find(":selected").text();
    var url = $(this).data('url');
    $.ajax({
        url: url,
        type: 'POST',
        headers: {
            'X-CSRF-TOKEN': jQuery('#token').val() // Ensure CSRF token is correct
        },
        data: { 'colcentre': colcentre }, // Pass the selected category ID to the server
        cache: false,
        success: function(data) {
            ...
            try {
                ...
                if (data && typeof data === 'object') {
                    ...
                    // Add "Please select..." placeholder as the first option
                    $('#vender').append(
                        $('<option>', {
                            value: '',          // Use an empty value for the placeholder
                            text: 'Please select...', // Display text
                    ...
                    $.each(data, function(key, value) {
                        $('#vender').append(
                            $('<option>', {
                                value: key,   // Use the key as the option value
                                text: value   // Use the value as the display text
                            })
                        );
                    });
                } else {
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group" id="vender-box">
                                ...
                                {{ Form::select('vender_id', [],$vendorId, array('class' => 'form-control select','id'=>'vender','data-url'=>route('bill.vender'),'required'=>'required')) }}
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        {{ Form::label('warehouse_id', __('Collection Centre'),['class'=>'form-label']) }}<x-required></x-required>
                                        {{ Form::select('warehouse_id', $warehouse,null, array('class' => 'form-control select2', 'data-url' => route('purchase.warehouse.vendors'), 'required'=>'required')) }}
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        {{ Form::date('purchase_date', null, ['class' => 'form-control', 'required' => 'required', 'max' => date('Y-m-d')]) }}
