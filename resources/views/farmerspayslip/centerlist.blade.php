<div class="modal-view">
    <h4 class="m-4">Farmers with Positive Balance</h4>
    <form id="bulk-payment-form" action="{{ route('payslip.farmer.center.bulk_pay_store') }}" method="POST">
        @csrf
        <table class="table table-striped">
            ...
            <tbody>
                @forelse ($venders as $vender)
                    <tr>
                        <td>
                            <input type="checkbox" name="vendor_ids[]" value="{{ $vender->id }}" class="vendor-checkbox" />
                        </td>
                        <td>
                            @can('show vender')
                            ...
                            @else
                                <a href="#" class="btn btn-outline-primary"> {{ AUth::user()->venderNumberFormat($vender->vender_id) }}</a>
                            @endcan
                        </td>
                        <td>{{ $vender->name }}</td>
                        <td>₦{{ number_format($vender->book_balance, 2, '.', ',') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="text-center">No vendors with positive balance found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <input type="hidden" value="{{preg_replace('/[^a-zA-Z0-9]/', '', $center)}}">
    </form>
</div>
...
<script>
    ...
    document.querySelectorAll('input[name="vendor_ids[]"]').forEach(function (checkbox) {
        checkbox.addEventListener('change', toggleSubmitButton);
    });

    // Enable or disable the submit button
    function toggleSubmitButton() {
        const checkboxes = document.querySelectorAll('input[name="vendor_ids[]"]:checked');
