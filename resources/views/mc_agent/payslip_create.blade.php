...
<div class="modal-view">
    <form id="bulk-payment-form" action="{{ route('mcagent.payslips.store') }}" method="POST">
        @csrf
 <div class="row mb-4">
        ...
        <table class="table table-striped">
            ...
            <tbody>
                @forelse ($riders as $rider)
                    <tr>
                        <td>
                            <input type="checkbox" name="agent_ids[]" value="{{ $rider->id }}" class="rider-checkbox" />
                        </td>
                        <td>
                            @can('show rider')
                                <a href="{{ route('rider.show', \Crypt::encrypt($rider->id)) }}" class="btn btn-outline-primary">
                                    SEB-AG-00{{ $rider->rider_id }}
                                </a>
                            @else
                                <a href="#" class="btn btn-outline-primary"> SEB-AG-00{{ $rider->rider_id }}</a>
                            @endcan
                        </td>
                        <td>{{ $rider->name }}</td>
                        <td>₦<input type="number" name="amounts[{{ $rider->id }}]" class="rider-amount" step="0.01" /></td>
                    </tr>
                @empty
                    <tr>
...
<script>
    ...
    document.querySelectorAll('input[name="agent_ids[]"]').forEach(function (checkbox) {
    ...
    function toggleSubmitButton() {
        const checked = document.querySelectorAll('input[name="agent_ids[]"]:checked');
