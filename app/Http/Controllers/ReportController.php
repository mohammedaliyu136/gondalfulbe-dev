...
public function purchaseDailyReport(Request $request)
{
    if (\Auth::user()->can('manage pos')) {
        ...
        // Base query with amount calculation
        $query = PurchaseProduct::select(
            'purchase_products.*',
            'purchases.purchase_date',
            'purchases.warehouse_id',
            'purchases.vender_id',
            'venders.name as vendor_name',
            DB::raw('purchase_products.quantity * purchase_products.price as amount') // Calculate amount
        )
        ...
        ->where('status', '>', 0);
        // Apply filters for warehouse and vendor
        if (!empty($request->warehouse)) {
            $query->where('purchases.warehouse_id', '=', $request->warehouse);
        }
        if (!empty($request->vendor)) {
            $query->where('purchases.vender_id', '=', $request->vendor);
        }

        // Filters for vendor name, quantity, and amount
        if (!empty($request->vendor_name)) {
            $query->where('venders.name', 'like', '%' . $request->vendor_name . '%');
        }
        if (!empty($request->quantity)) {
            $query->where('purchase_products.quantity', '>=', $request->quantity);
        }
        if (!empty($request->amount)) {
            $query->where(DB::raw('purchase_products.quantity * purchase_products.price'), '>=', $request->amount);
        }
        ...
        $query->whereBetween('purchases.purchase_date', [$first_date, $end_date]);

        // Group and calculate totals per day
        ...
        if (!empty($purchases) && count($purchases) > 0) {
            foreach ($purchases as $day => $onepurchase) {
                ...
                foreach ($onepurchase as $purchase) {
                    $totals += $purchase->amount; // Use pre-calculated amount
                }
                $total[$day] = $totals;
            }
        }
        ...
        $arrDuration = [];
        $data = [];
        $previous_days = strtotime('-1 month +1 day');
        for ($i = 0; $i < 30; $i++) {
        ...
        // Get warehouse and vendor names for the filter
        $filter['startDate'] = $first_date;
        $filter['endDate'] = $end_date;

        $warehouses = warehouse::where('id', '=', $request->warehouse)
            ->where('created_by', \Auth::user()->creatorId())
            ->first();
        $filter['warehouse'] = !empty($warehouses) ? $warehouses->name : '';

        $vendors = Vender::where('id', '=', $request->vendor)
            ->where('created_by', \Auth::user()->creatorId())
            ->first();
        $filter['vendor'] = !empty($vendors) ? $vendors->name : '';

        // Calculate total quantity and total amount
        $totalQuantity = $query->sum('purchase_products.quantity');
        $totalAmount = $query->sum(DB::raw('purchase_products.quantity * purchase_products.price'));

        // Pass filtered results to view
        $filteredVendors = $query->get(); // Get filtered vendors

        return view('report.daily_purchase', compact('warehouse', 'vendor', 'arrDuration', 'data', 'filter', 'totalQuantity', 'totalAmount', 'filteredVendors'));
    } else {
        return redirect()->back()->with('error', __('Permission denied.'));
    }
}
