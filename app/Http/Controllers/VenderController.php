...
class VenderController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('create vender'))
        {
            ...
            $cooperatives = Cooperative::where('created_by', \Auth::user()->creatorId())->get();
            $monnify = new Monnify();
            ...
            return view('vender.create', compact('customFields', 'warehouses', 'cooperatives', 'bankList'));        
   ...
   public function store(Request $request)
{
    ...
    public function edit($id)
    {
        if(\Auth::user()->can('edit vender'))
        {
            ...
            return view('vender.edit', compact('vender', 'customFields', 'bankList', 'selectedBankCode', 'warehouses', 'selectedWarehouseId', 'cooperatives'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function destroy(Vender $vender)
    {
        if(\Auth::user()->can('delete vender'))
        {
            if($vender->created_by == \Auth::user()->creatorId())
            {
                ...
                // Check if the vender has any purchase records
                $hasPurchase = Purchase::where('vender_id', $vender->id)->exists();
                if ($hasPurchase) {
                    $vender->is_active = 0;
                    $vender->save();
                    return redirect()->back()->with('error', __('Farmer cannot be deleted due to existing purchase records. The rider has been deactivated instead.'));
                }
                
                // Check if the vender has any payslip records
                $hasPayslips = PaySlipFarmerBatchItem::where('vender_id', $vender->id)->exists();
                if ($hasPayslips) {
                    $vender->is_active = 0;
                    $vender->save();
                    return redirect()->back()->with('error', __('Farmer cannot be deleted due to existing payslip records. The rider has been deactivated instead.'));
                }
