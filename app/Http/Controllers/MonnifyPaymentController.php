...
class MonnifyPaymentController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if(!$this->live){
            ...
            $this->transaction_url = "https://sandbox.monnify.com/api/v2/transactions/";
        }else{
            $this->auth_url = "https://api.monnify.com/api/v1/auth/login";
            $this->transaction_url = "https://api.monnify.com/api/v2/transactions/";
        }
    ...
    public function paymentConfig()
    {
        ...
        $this->is_enabled = isset($payment_setting['is_monnify_enabled']) ? $payment_setting['is_monnify_enabled'] : 'off';
        
        return $this;
    }

    public function planPayWithMonnify(Request $request)
    {
        $this->paymentConfig();
    ...
    public function RetainerPayWithMonnify(Request $request)
    {
        ...
        if ($retainer) {
            ...
            if ($price > 0) {
                ...
                $res_data['name']       = $retainer->customer->name;
                $res_data['phone']       = $retainer->customer->phone;
    ...
    public function invoicePayWithMonnify(Request $request)
    {
        ...
        if ($invoice) {
            ...
            if ($price > 0) {
                ...
                $res_data['name']       = $invoice->customer->name;
                $res_data['phone']       = $invoice->customer->phone;
                $res_data['total_price'] = (int)$price;
                $res_data['currency']    = $setting['site_currency'];
                $res_data['flag']        = 1;
    ...
    public function getRetainerPaymentStatus(Request $request, $retainer_id, $amount, $pay_id)
    {
        ...
        if ($retainer) {
            $api_key = $this->public_key;       // Replace with your API Key
            $client_secret = $this->secret_key; // Replace with your Client 
            // Create the base64-encoded authorization string
            $auth_string = base64_encode("$api_key:$client_secret");
            ...
            // Send empty JSON data as per the request sample
            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode([]));
            $auth_response = curl_exec($ch);
            if (curl_errno($ch)) {
                die('Error authenticating: ' . curl_error($ch));
            }
            ...
            if (isset($auth_data['responseBody']['accessToken'])) {
                ...
                die("Failed to retrieve access token: " . $auth_response);
                
            }
            curl_close($ch);
            
            // Step 2: Use Access Token for Subsequent API Requests
            $pay_id = urlencode($pay_id); // Replace with your payment ID
            $transaction_url = $this->transaction_url.$pay_id;
            ...
            $response = curl_exec($ch);
             ...
             if (curl_errno($ch)) {
                echo 'Error fetching transaction: ' . curl_error($ch);
            } else {
                $transaction_data = json_decode($response, true);
                
                if (isset($transaction_data['responseBody']['paymentStatus']) && $transaction_data['responseBody']['paymentStatus'] == 'PAID') {
                    $payments = RetainerPayment::create(
                            [
                                ...
                                'payment_method' => 1,
                                'order_id' => $orderID,
                                'payment_type' => __('Monnify'),
                                'receipt' => '',
                                'reference' => $transaction_data['responseBody']['transactionReference'],
                                'description' => __('Self Assessment') . ' ' . Utility::retainerNumberFormat($settings, $retainer->retainer_id),
    ...
    public function getInvoicePaymentStatus(Request $request, $invoice_id, $amount, $pay_id)
    {
        ...
        return redirect()->route('pay.invoice', encrypt($invoice->id))->with('error', __('Transaction could not be verified.'));
    }
