    ...
    public function pos_dashboard_index()
{
    ...
    // Prepare POS data for non-admin users
    $pos_data = [
        ...
        'dailyQuantity' => \DB::table('purchase_products')
            ->whereDate('created_at', now()->toDateString())
            ->sum('quantity') ?? 0,
        'dailyPrice' => \DB::table('purchase_products')
            ->whereDate('created_at', now()->toDateString())
            ->selectRaw('sum(price * quantity) as total_price')
            ->value('total_price') ? '₦' . number_format(
...
'weeklyQuantity' => \DB::table('purchase_products')
...
'weeklyPrice' => \DB::table('purchase_products')
    ->whereBetween('created_at', [
        Carbon::now()->startOfWeek(Carbon::FRIDAY),
        Carbon::now()->endOfWeek(Carbon::FRIDAY),
    ])
    ...
    ->value('total_price') ? '₦' . number_format(
        ...
        'monthlyQuantity' => \DB::table('purchase_products')
            ->whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])
            ->sum('quantity') ?? 0,
        'monthlyPrice' => \DB::table('purchase_products')
            ->whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])
            ->selectRaw('sum(price * quantity) as total_price')
            ->value('total_price') ? '₦' . number_format(
                \DB::table('purchase_products')
