...
class SystemController extends Controller
{
    ...
    public function saveCompanyPaymentSettings(Request $request)
    {
        ...
        if (isset($request->company_ozow_payment_is_enabled) && $request->company_ozow_payment_is_enabled == 'on') {
         ...
         //Monnify
            if (isset($request->is_monnify_enabled) && $request->is_monnify_enabled == 'on') {
            $request->validate([
                'monnify_mode' => 'required|string',
                'monnify_public_key' => 'required|string',
                'monnify_secret_key' => 'required|string',
                'monnify_contract_code' => 'required|string',
                'monnify_source_account' => 'required|string',
            ]);
            $post['monnify_mode'] = $request->monnify_mode;
            $post['is_monnify_enabled'] = $request->is_monnify_enabled;
            $post['monnify_public_key'] = $request->monnify_public_key;
            $post['monnify_secret_key'] = $request->monnify_secret_key;
            $post['monnify_contract_code'] = $request->monnify_contract_code;
            $post['monnify_source_account'] = $request->monnify_source_account;
            } else {
                $post['is_monnify_enabled'] = 'off';
            }
