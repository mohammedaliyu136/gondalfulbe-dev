...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('create employee'))
        {
            ...
            return view('employee.create', compact('employees', 'employeesId', 'departments', 'designations', 'documents', 'branches', 'company_settings', 'bankList'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create employee'))
        {
            ...
            if ($request->hasFile('passport')) {
                ...
                $path = public_path('uploads/employees_passport');
                if (!file_exists($path)) {
                    mkdir($path, 0777, true);
                }
            
                // Move file
                $request->passport->move($path, $imageName);
            
                // Save to employee
                $employee->passport = $imageName;
                $employee->save(); 
            }
...
//            $employeesId  = \Auth::user()->employeeIdFormat($employee->employee_id);
            ...
            return view('employee.edit', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents','departmentData', 'bankList'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = \Validator::make(
            ...
            [
                ...
                'address'   => 'required',
                'passport'  => 'nullable|image|mimes:jpg,jpeg,png|max:2048'  // NOT required on update
            ]
        );
        ...
        */
        if ($request->hasFile('passport')) {
            ...
            if (!empty($employee->passport)) {
                $oldPath = public_path('uploads/employees_passport/' . $employee->passport);
        ...
        /*
        |--------------------------------------------------------------------------
        | HANDLE OTHER DOCUMENT UPLOADS (your original logic)
        |--------------------------------------------------------------------------
        */
        if ($request->document) {
            foreach ($request->document as $key => $document) {
        ...
        /*
        |--------------------------------------------------------------------------
        | UPDATE EMPLOYEE & USER RECORDS
        |--------------------------------------------------------------------------
        */
        $input = $request->except(['passport']); // avoid conflicts
        ...
        */
        ...
        return redirect()
            ->route('employee.show', \Crypt::encrypt($employee->id))
    ...
    public function printIdCard($id)
    {
        ...
        return view('employee.print_id_card', compact('employee'));
    }
    ...
    public function showVerify($uuid)
    {
        ...
        // If no matching employee, show invalid page
        if (!$employee) {
            return view('employee.invalid');
        }
